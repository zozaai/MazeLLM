"""Llama-3.2-1B-Instruct-specific formatting: neutral expert turns -> chat examples.

Identical to `scripts/llama-3.2-3b/llama_format.py`: `unsloth/Llama-3.2-1B-Instruct`
and `unsloth/Llama-3.2-3B-Instruct` ship byte-identical `chat_template.jinja`
(diffed both tokenizers' `chat_template` directly — not assumed), so everything
verified there applies unchanged here:
